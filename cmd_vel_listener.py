...
class CmdVelListener(Node):
    ...
    def cmd_vel_callback(self, msg):
        # Reducem viteza (scalare mai mica)
        pwm = int(abs(msg.linear.x) * 255 / 1.0)  # Scadere viteza (1.0 in loc de 0.5)
        pwm = constrain(pwm, 0, 255)

        # Comportament roti conform cerintelor
        if msg.linear.x > 0.1:
            # Mers inainte: dreapta inainte, stanga inapoi
            self.send_command(pwm, pwm, False, True)
        elif msg.linear.x < -0.1:
            # Mers inapoi: dreapta inapoi, stanga inapoi
            self.send_command(pwm, pwm, True, True)
        elif msg.angular.z > 0.1:
            # Rotire dreapta: dreapta inapoi, stanga inainte
            self.send_command(pwm, pwm, True, False)
        elif msg.angular.z < -0.1:
            # Rotire stanga: dreapta inainte, stanga inapoi
